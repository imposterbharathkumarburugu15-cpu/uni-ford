...
export function DecisionWorkspace({ conflictId }: DecisionWorkspaceProps) {
  ...
  const [selectedValue, setSelectedValue] = useState(conflict?.recommendation ?? "");
  const [customValue, setCustomValue] = useState("");
  const [reason, setReason] = useState("");
  const [busy, setBusy] = useState(false);
  const [prevConflictId, setPrevConflictId] = useState(conflictId);

  if (conflict && conflict.id !== prevConflictId) {
    setPrevConflictId(conflict.id);
    setSelectedValue(conflict.recommendation);
